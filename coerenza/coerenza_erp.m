function coerenza_erp(outputMatlabRoot, outRoot, overwriteOutput, sourceRoot, emgLowpassHz, surrogateN, forceScalpOnly, analysisFsHz, beepWindowSec)
%COERENZA_ERP
% Coerenza cortico-sinergica ROI-EMG in modalita' ERP timecourses ONLY.
% Non modifica NET: legge solo file gia' prodotti.
%
% English overview:
% - consumes aligned MFF outputs produced by allineamento_eeg_emg;
% - builds trial windows from reconstructed beep events with NET-like epoch;
% - uses NET ERP sensor trial timecourses (erp_timecourses_sensor.mat);
% - aligns EMG trials to the same epoch/trial definition;
%
% Input default:
%   outputMatlabRoot = ../batch_otb_mff/output_matlab
%   outRoot          = ./coerenza_motoria_erp
%
% Requisiti:
%   - report.json prodotto da allineamento_eeg_emg.m
%   - plugin EEGLAB MFFMatlabIO disponibile (pop_mffimport)

if nargin < 1 || isempty(outputMatlabRoot)
    scriptDir = fileparts(mfilename('fullpath'));
    outputMatlabRoot = fullfile(fileparts(scriptDir), 'batch_otb_mff', 'output_matlab');
end
if nargin < 2 || isempty(outRoot)
    outRoot = fullfile(fileparts(mfilename('fullpath')), 'coerenza_motoria_erp');
end
...
if nargin < 5 || isempty(emgLowpassHz)
    emgLowpassHz = 10;
end
if ~isscalar(emgLowpassHz) || ~isfinite(emgLowpassHz) || emgLowpassHz <= 0
    error('emgLowpassHz deve essere uno scalare positivo (Hz).');
end
emgLowpassHz = double(emgLowpassHz);
if nargin < 6 || isempty(surrogateN)
    surrogateN = 200;
end
if ~isscalar(surrogateN) || ~isfinite(surrogateN) || surrogateN < 10
    error('surrogateN deve essere uno scalare >= 10.');
end
surrogateN = round(double(surrogateN));
if nargin < 7 || isempty(forceScalpOnly)
    forceScalpOnly = false;
end
forceScalpOnly = logical(forceScalpOnly); %#ok<NASGU>
if nargin < 8 || isempty(analysisFsHz)
    analysisFsHz = 1000; % Keep NET sampling to preserve timing fidelity
end
if ~isscalar(analysisFsHz) || ~isfinite(analysisFsHz) || analysisFsHz <= 0
    error('analysisFsHz deve essere uno scalare positivo (Hz).');
end
analysisFsHz = double(analysisFsHz);
if nargin < 9
    beepWindowSec = 2.0; % Default NET-like post-trigger window (+2000 ms)
end
if ~isempty(beepWindowSec)
    if ~isscalar(beepWindowSec) || ~isfinite(beepWindowSec) || beepWindowSec <= 0
        error('beepWindowSec deve essere vuoto oppure uno scalare positivo (secondi).');
    end
    beepWindowSec = double(beepWindowSec);
end
epochPreSec = 0.199; % NET pre=-200 ms yields first ERP sample at -199 ms
...
initEeglab();
roiCfgScalp = defaultOrtegaScalpRoiConfig();
roiCfg = roiCfgScalp;
erpDb = indexErpSensorFiles(sourceRoot, outputMatlabRoot);
fprintf('EMG preprocessing: bandpass 20-450 Hz, notch 50:50:400 Hz, rectify, low-pass %.1f Hz.\n', emgLowpassHz);
fprintf('Surrogate test: N=%d, BH-FDR q<0.05.\n', surrogateN);
fprintf('Analysis sampling rate (Ortega-style fixed): %.1f Hz.\n', analysisFsHz);
if isempty(beepWindowSec)
    fprintf('Trial definition: beep->start (evento-evento).\n');
    fprintf('Trial max duration clamp: 3.000 s (3000 ms @ 1000 Hz).\n');
else
    fprintf('Trial definition: epoca NET-like [%.0f, +%.0f] ms attorno a beep.\n', -epochPreSec*1000, beepWindowSec*1000);
end
fprintf('EEG mode: ERP timecourses only.\n');
...
summaryHeader = {'pair','phase','status','message','n_trials','n_eeg_ch','n_emg_ch', ...
    ...
    'emg_bp_low_hz','emg_bp_high_hz','emg_env_lowpass_hz', ...
...
for i = 1:numel(reports)
    ...
    try
        ...
        % Load aligned EEG and derive trial windows (beep->start or fixed beep window).
        [eegData, eegFs, eegLabels, eegChanlocs] = loadMffEeg(R.output_mff);
        pairSide = inferSideLabel(pairName);
        roiCfgScalpRun = sideAwareOrtegaScalpRoiConfig(roiCfgScalp, pairSide, eegLabels, eegChanlocs);
        trials = buildTrialsFromMffEvents(R.output_mff, eegFs, size(eegData,2), beepWindowSec, epochPreSec);
        if isempty(trials)
            if isempty(beepWindowSec)
                error('Nessun trial beep/start valido trovato in MFF');
            else
                error('Nessun trial beep+finestra fissa valido trovato in MFF');
            end
        end
        ...
        [erpSensorFile, erpSensorStatus] = resolveErpSensorFile(R, pairName, phase, erpDb);
        if isempty(erpSensorFile) || ~isfile(erpSensorFile)
            error('ERP sensor timecourses non trovato per %s/%s', pairName, phase);
        end

        roiCfg = roiCfgScalpRun;
        [eegTrialsRaw, eegLabelsErp, erpFs] = loadErpSensorTrialCells(erpSensorFile);
        eegTrialsRaw = resampleTrialCellsIfNeeded(eegTrialsRaw, erpFs, analysisFsHz);
        eegFsUse = analysisFsHz;
        nEegPtsUse = max(1, round((size(eegData,2)-1) * eegFsUse / eegFs) + 1);
        emgAligned = preprocessAndAlignEmg(emgRaw, emgFs, trA, trB, eegFsUse, nEegPtsUse, emgLowpassHz);
        trialsUse = secondsToTrials(trialsSec, eegFsUse, size(emgAligned,2));
        [emgTrialsAll, trialInfoAll] = sliceEmgTrials(emgAligned, trialsUse);
        [eegTrialsRaw, emgTrials, trialInfo, pairMsg] = pairAndTrimTrials(eegTrialsRaw, emgTrialsAll, trialInfoAll);
        if isempty(eegTrialsRaw)
            error('Nessun trial ERP-EMG valido dopo allineamento');
        end
        [eegTrials, eegSelIdx, eegSelLabels, roiMap] = selectMotorRoiChannels(eegTrialsRaw, eegLabelsErp, roiCfg);
        eegMode = 'erp_timecourses';
        sourceFile = erpSensorFile;
        sourceStatus = ['erp_' erpSensorStatus];
        if ~isempty(pairMsg)
            sourceStatus = [sourceStatus '|' pairMsg];
        end
        deltaBetaMean = NaN;
        deltaBetaPeak = NaN;
        deltaBetaAuc  = NaN;
        ...
        [cohMean, f, metrics] = computeCorticoSynergyCoherence( ...
            eegTrials, Htrials, eegFsUse, eegSelLabels, emgTrials, struct('surrogateN', surrogateN));
...
summaryCsv = fullfile(outRoot, 'summary_coerenza_motoria_erp.csv');
writeCsv(summaryCsv, summaryHeader, summaryRows);
writePrePostBetaSummary(fullfile(outRoot, 'summary_beta_pre_post_motoria_erp.csv'), summaryRows, summaryHeader);
finalTableCsv = fullfile(outRoot, 'summary_tabella_finale_erp.csv');
writeSourceFinalTable(finalTableCsv, summaryRows, summaryHeader);
fprintf('\nSummary: %s\n', summaryCsv);
fprintf('Tabella finale: %s\n', finalTableCsv);
fprintf('Nota: nessuna modifica a NET. Modalita'' ERP timecourses only.\n');
end
...
function [data, fs, labels, chanlocs] = loadMffEeg(mffPath)
...
if contains(s, '_dx_') || endsWith(s,'_dx') || contains(s,'reachdx') || contains(s,'destra') || contains(s,'right')
    side = 'dx';
elseif contains(s, '_sx_') || endsWith(s,'_sx') || contains(s,'reachsx') || contains(s,'sin') || contains(s,'left')
    side = 'sx';
else
    side = '';
end
...
function erpDb = indexErpSensorFiles(sourceRoot, outputMatlabRoot)
erpDb = struct('mff', {{}}, 'erp', {{}}, 'keys', {{}});
roots = {};
...
if nargin >= 2 && ~isempty(outputMatlabRoot) && isfolder(outputMatlabRoot)
    ...
    for up = 1:8
        ...
        d = dir(fullfile(p, 'EEG_NET*'));
        for k = 1:numel(d)
            if d(k).isdir
                roots{end+1} = fullfile(d(k).folder, d(k).name); %#ok<AGROW>
            end
        end
        pp = fileparts(p);
        if strcmp(pp, p)
...
for i = 1:numel(roots)
    xlsList = dir(fullfile(roots{i}, '*_dataset.xlsx'));
    for ix = 1:numel(xlsList)
        xlsPath = fullfile(xlsList(ix).folder, xlsList(ix).name);
        try
            [~,~,X] = xlsread(xlsPath, 1);
        catch
            continue;
        ...
        eegCol = 1;
        for c = 1:size(X,2)
            h = lower(strtrim(string(X{1,c})));
            if contains(h, 'eeg') && contains(h, 'file')
                eegCol = c;
                break;
            end
        end
        for r = 2:size(X,1)
            mffPath = char(string(X{r, eegCol}));
            if isempty(strtrim(mffPath))
                continue;
            end
            erpPath = fullfile(roots{i}, sprintf('dataset%d', r-1), 'eeg_signal', 'erp_results', 'erp_timecourses_sensor.mat');
            if ~isfile(erpPath)
                continue;
            end
            k = lower(strtrim(string(mffPath)));
            if any(strcmp(erpDb.keys, k))
                continue;
            end
            erpDb.mff{end+1} = mffPath; %#ok<AGROW>
            erpDb.erp{end+1} = erpPath; %#ok<AGROW>
            erpDb.keys{end+1} = k; %#ok<AGROW>
        end
    end
end
end

function [erpFile, status] = resolveErpSensorFile(R, pairName, phase, erpDb)
erpFile = '';
status = 'not_found';

candidateFields = {'erp_sensor_file','erp_file','erp_timecourses_sensor'};
for i = 1:numel(candidateFields)
    f = candidateFields{i};
    if isfield(R,f) && ischar(R.(f)) && isfile(R.(f))
        erpFile = R.(f);
...
if isempty(erpDb.mff)
    status = 'index_empty';
    return;
end

if isfield(R,'output_mff') && ischar(R.output_mff)
    k = lower(strtrim(string(R.output_mff)));
    ix = find(strcmp(erpDb.keys, k), 1, 'first');
    if ~isempty(ix)
        erpFile = erpDb.erp{ix};
        status = 'mapped_by_mff';
        return;
    end

    [~,bn,be] = fileparts(char(R.output_mff));
    b = lower([bn be]);
    scores = zeros(numel(erpDb.mff),1);
    for i = 1:numel(erpDb.mff)
        if contains(lower(erpDb.mff{i}), b)
            scores(i) = scores(i) + 10;
        end
        toks = tokenizePair(pairName, phase);
        for t = 1:numel(toks)
            if contains(lower(erpDb.mff{i}), toks{t})
                scores(i) = scores(i) + 1;
            end
        end
    end
    ...
    if mx >= 10
        erpFile = erpDb.erp{ix};
        status = sprintf('mapped_by_name_%d', mx);
        return;
    end
end

status = 'index_not_matched';
end

function [eegTrials, labels, fs] = loadErpSensorTrialCells(erpFile)
S = load(erpFile);
if ~isfield(S, 'erp_sensor')
    error('Campo erp_sensor mancante in %s', erpFile);
end
E = S.erp_sensor;
if ~isfield(E, 'trials') || isempty(E.trials)
    error('erp_sensor.trials mancante/vuoto in %s', erpFile);
end

labels = {};
if isfield(E, 'label') && ~isempty(E.label)
    labels = cellstr(string(E.label(:)))';
end

t = [];
if isfield(E, 'time_axis') && ~isempty(E.time_axis)
    t = double(E.time_axis(:));
elseif isfield(E, 'time_ms') && ~isempty(E.time_ms)
    t = double(E.time_ms(:));
elseif isfield(E, 'time') && ~isempty(E.time)
    t = double(E.time(:));
end
if numel(t) >= 2
    dt = median(diff(t));
...
X = double(E.trials);
if ndims(X) ~= 3
    error('erp_sensor.trials deve essere 3D in %s', erpFile);
end
...
for i = 1:size(permList,1)
    Y = permute(X, permList(i,:)); % Y: trial x ch x time (atteso)
    okLab = (nLab == 0) || (size(Y,2) == nLab);
    okTim = (nTime == 0) || (size(Y,3) == nTime);
    if okLab && okTim
        Xp = Y;
        break;
    end
...
if isempty(Xp)
    % Fallback conservativo: assume trials x channels x time.
    Xp = X;
end
...
eegTrials = cell(1, nTrials);
for i = 1:nTrials
    eegTrials{i} = squeeze(Xp(i,:,:));
end
end

function trialsOut = resampleTrialCellsIfNeeded(trialsIn, fsIn, fsOut)
if isempty(trialsIn) || abs(fsIn - fsOut) < 1e-12
    trialsOut = trialsIn;
    return;
end
trialsOut = cell(size(trialsIn));
for i = 1:numel(trialsIn)
    X = double(trialsIn{i});
    tIn = (0:size(X,2)-1) / fsIn;
    nOut = max(1, round((size(X,2)-1) * fsOut / fsIn) + 1);
    tOut = (0:nOut-1) / fsOut;
    Y = zeros(size(X,1), nOut);
    for r = 1:size(X,1)
        Y(r,:) = interp1(tIn, X(r,:), tOut, 'linear', 'extrap');
    end
    trialsOut{i} = Y;
end
end

function [emgTrials, trialInfo] = sliceEmgTrials(emgAligned, trials)
emgTrials = {};
trialInfo = zeros(0,3); % idx, start, end
for i = 1:size(trials,1)
    i1 = trials(i,1);
    i2 = trials(i,2);
    if i2 <= i1 || i1 < 1 || i2 > size(emgAligned,2)
        continue;
    end
    m = emgAligned(:, i1:i2);
    if size(m,2) < 50
...
function [eegOut, emgOut, trialInfoOut, note] = pairAndTrimTrials(eegIn, emgIn, trialInfoIn)
eegOut = {};
emgOut = {};
trialInfoOut = zeros(0,3);
note = '';
if isempty(eegIn) || isempty(emgIn)
    return;
end
nPair = min(numel(eegIn), numel(emgIn));
if nPair < numel(eegIn) || nPair < numel(emgIn)
    note = sprintf('paired_first_%d_of_%d_%d', nPair, numel(eegIn), numel(emgIn));
end
for i = 1:nPair
    e = double(eegIn{i});
    m = double(emgIn{i});
    L = min(size(e,2), size(m,2));
    if L < 50
        continue;
    end
    eegOut{end+1} = e(:,1:L); %#ok<AGROW>
    emgOut{end+1} = max(m(:,1:L), 0); %#ok<AGROW>
    if i <= size(trialInfoIn,1)
        trialInfoOut(end+1,:) = trialInfoIn(i,:); %#ok<AGROW>
    else
        trialInfoOut(end+1,:) = [i, NaN, NaN]; %#ok<AGROW>
    end
end
end
...
function cfg = sideAwareOrtegaScalpRoiConfig(baseCfg, movementSide, labels, chanlocs)
cfg = baseCfg;
if nargin < 2 || isempty(movementSide)
    return;
end
side = lower(char(string(movementSide)));
if ~strcmp(side, 'sx')
    % Default template is left/contralateral (used for dx).
    return;
end

leftNames = {'fc5','fc1','fz','cz','c3','t7','cp5','cp1'};
rightNames = {'fc6','fc2','fz','cz','c4','t8','cp6','cp2'};

for r = 1:numel(cfg)
    nm = lower(char(string(cfg(r).name)));
    ix = find(strcmp(leftNames, nm), 1);
    if ~isempty(ix)
        cfg(r).name = rightNames{ix};
    end
    if strcmp(nm, 'fz') || strcmp(nm, 'cz')
        continue;
    end
    labs = cfg(r).labels;
    for k = 1:numel(labs)
        labs{k} = mirrorChannelLabelByGeometry(labs{k}, labels, chanlocs);
    end
    cfg(r).labels = labs;
    if isfield(cfg, 'mni_xyz') && numel(cfg(r).mni_xyz) >= 1 && isfinite(cfg(r).mni_xyz(1))
        cfg(r).mni_xyz(1) = -cfg(r).mni_xyz(1);
    end
end
end

function outLabel = mirrorChannelLabelByGeometry(inLabel, labels, chanlocs)
outLabel = char(string(inLabel));
if isempty(labels) || isempty(chanlocs)
    return;
end
labList = string(labels);
idx = find(strcmpi(labList, string(inLabel)), 1);
if isempty(idx)
    return;
end

[X, okX] = chanlocNumeric(chanlocs, 'X');
[Y, okY] = chanlocNumeric(chanlocs, 'Y');
[Z, okZ] = chanlocNumeric(chanlocs, 'Z');
if ~(okX && okY && okZ)
    return;
end
if idx > numel(X) || ~isfinite(X(idx)) || ~isfinite(Y(idx)) || ~isfinite(Z(idx))
    return;
end
...
yr = Y(idx);
zr = Z(idx);
% EEGLAB chanlocs with nose +X: left/right is encoded mainly on Y sign.
d = (X - xr).^2 + (Y + yr).^2 + (Z - zr).^2;
cand = find(isfinite(d));
cand(cand == idx) = [];
if isempty(cand)
    return;
end
if isfinite(yr) && abs(yr) > 1e-6
    opp = cand(sign(Y(cand)) * sign(yr) < 0);
    if ~isempty(opp)
        cand = opp;
    end
end
[~, ii] = min(d(cand));
j = cand(ii);
outLabel = char(string(labList(j)));
end

function [v, ok] = chanlocNumeric(chanlocs, fieldName)
n = numel(chanlocs);
v = nan(1,n);
ok = true;
for i = 1:n
    if ~isfield(chanlocs(i), fieldName)
        ok = false;
        return;
    end
    x = chanlocs(i).(fieldName);
    if isempty(x)
        v(i) = NaN;
    elseif isnumeric(x)
        v(i) = double(x(1));
    else
        v(i) = str2double(char(string(x)));
    end
end
end

function Y = resampleContinuousIfNeeded(X, fsIn, fsOut)
if abs(fsIn - fsOut) < 1e-12
    Y = X;
    return;
end
tIn = (0:size(X,2)-1) / fsIn;
...
function trials = buildTrialsFromMffEvents(mffPath, fs, nPts, beepWindowSec, epochPreSec)
[times, codes] = readMffEvents(mffPath);
% Trials can be defined as:
% 1) beep -> first start before next beep (default), or
% 2) NET-like fixed epoch (beep-epochPreSec) -> (beep+beepWindowSec).
beeps = sort(times(strcmp(codes,'beep')));
trials = zeros(0,2);
minTrialSamples = minReachTrialSamples(fs);
maxTrialSamples = maxReachTrialSamples(fs);
if isempty(beeps)
    return;
end

if nargin >= 4 && ~isempty(beepWindowSec) && isfinite(beepWindowSec) && beepWindowSec > 0
    if nargin < 5 || isempty(epochPreSec) || ~isfinite(epochPreSec) || epochPreSec < 0
        epochPreSec = 0;
    end
    recEndSec = (nPts - 1) / fs;
    for i = 1:numel(beeps)
        s = beeps(i) - epochPreSec;
        e = beeps(i) + beepWindowSec;
        % Mirror NET behavior: discard trials that do not fully fit the epoch.
        if s < 0 || e > recEndSec
            continue;
        end
        i1 = floor(s*fs) + 1;
...
for i = 1:numel(beeps)
    ...
    i2Raw = min(nPts, ceil(e*fs));
    % Clamp overly long beep->start windows to 3 s (3000 ms @ fs=1000 Hz)
    % to keep coherence estimates comparable across trials/runs.
    i2 = min(i2Raw, i1 + maxTrialSamples - 1);
    if i2 <= i1
        continue;
    end
...
function n = maxReachTrialSamples(fs)
% Enforce a maximum reaching duration of 3.00 s in every sampling space.
n = max(1, round(3.00 * fs));
end
...
[~,~,ext] = fileparts(lower(otbPath));
if strcmp(ext,'.mat')
    [emgRaw, fs, labels] = readMatBipolar10(otbPath);
    return;
end
...
function [emgRaw, fs, labels] = readMatBipolar10(matPath)
S = load(matPath);
if ~isfield(S,'Data') || ~iscell(S.Data) || isempty(S.Data) || ~isnumeric(S.Data{1})
    error('MAT EMG non valido: Data mancante/non numerica (%s)', matPath);
end

raw = double(S.Data{1});
if ndims(raw) ~= 2
    error('MAT EMG non valido: Data deve essere 2D (%s)', matPath);
end
...
if isfield(S,'SamplingFrequency') && isnumeric(S.SamplingFrequency) && isfinite(S.SamplingFrequency)
    fs = double(S.SamplingFrequency(1));
elseif isfield(S,'Time') && iscell(S.Time) && ~isempty(S.Time) && isnumeric(S.Time{1}) && numel(S.Time{1}) >= 2
    dt = median(diff(double(S.Time{1}(:))));
    if ~(isfinite(dt) && dt > 0)
        error('MAT EMG non valido: impossibile stimare fs da Time (%s)', matPath);
    end
    fs = 1/dt;
else
    error('MAT EMG non valido: SamplingFrequency/Time mancanti (%s)', matPath);
end
...
if ~isempty(desc)
    keep = true(numel(desc),1);
    dlow = lower(desc);
    keep = keep & ~contains(dlow,'aux');
    keep = keep & ~contains(dlow,'trigger');
    keep = keep & ~contains(dlow,'multiple');
    keep = keep & ~contains(dlow,'sync');
    idx = find(keep);
    idx = idx(idx <= nChannels);
    if isempty(idx)
        idx = 1:min(10,nChannels);
    else
        idx = idx(1:min(10,numel(idx)));
    end
    labels = cell(numel(idx),1);
    for k = 1:numel(idx)
        nm = regexprep(strtrim(char(desc(idx(k)))), '\s+', '_');
        labels{k} = sprintf('BIP_%02d_%s', k, nm);
    end
else
    idx = 1:min(10,nChannels);
    labels = arrayfun(@(k) sprintf('BIP_%02d',k), 1:numel(idx), 'UniformOutput', false).';
end

emgRaw = data(idx, :);
end

function emgAligned = preprocessAndAlignEmg(emgRaw, fsEmg, a, b, fsEeg, nEegPts, emgLowpassHz)
...
for ch = 1:size(emgRaw,1)
    s = double(emgRaw(ch,:));
    s = preprocessEmgSignal(s, fsEmg, emgLowpassHz);
...
function s = preprocessEmgSignal(s, fs, emgLowpassHz)
s = s(:)';
...
s = abs(s);
s = applyLowpass(s, fs, min(emgLowpassHz, fs/4));
end
...
function cfg = defaultOrtegaScalpRoiConfig()
% ROI scalp in stile Ortega-Auriol et al. (CSC su canali motori 10-20):
% FC5, FC1, Fz, Cz, C3, T7, CP5, CP1.
%
% Qui usiamo una mappatura approssimata su EGI256 (template) ottenuta per
% vicinanza geometrica con i template sensori disponibili in NET.
cfg = struct('name', {}, 'labels', {}, 'mni_xyz', {});

cfg(1).name = 'fc5';
cfg(1).mni_xyz = [-50 15 55];
cfg(1).labels = {'E56'};

cfg(2).name = 'fc1';
cfg(2).mni_xyz = [-25 20 70];
cfg(2).labels = {'E24'};

cfg(3).name = 'fz';
cfg(3).mni_xyz = [0 35 60];
cfg(3).labels = {'E14'};

cfg(4).name = 'cz';
cfg(4).mni_xyz = [0 -20 80];
cfg(4).labels = {'E9'};

cfg(5).name = 'c3';
cfg(5).mni_xyz = [-40 -20 65];
cfg(5).labels = {'E59'};

cfg(6).name = 't7';
cfg(6).mni_xyz = [-65 -20 10];
cfg(6).labels = {'E68'};

cfg(7).name = 'cp5';
cfg(7).mni_xyz = [-60 -45 25];
cfg(7).labels = {'E75'};

cfg(8).name = 'cp1';
cfg(8).mni_xyz = [-30 -45 70];
cfg(8).labels = {'E80'};
end
...
cfg(3).labels = {'E90','E91','E92','E103','E146'};

cfg(4).name = 'pmd_r';
cfg(4).mni_xyz = [26 -6 58];
cfg(4).labels = {'E239','E240','E235','E237','E230','E232','E233','E229'};

cfg(5).name = 'sma_l';
cfg(5).mni_xyz = [-6 -6 58];
cfg(5).labels = {'E72','E133','E120','E112','E43','E17'};
...
cfg(10).labels = {'E165','E156','E147','E157','E158'};
end
...
if isfield(opts,'surrogateN') && ~isempty(opts.surrogateN) && isfinite(opts.surrogateN)
    surrN = max(10, round(double(opts.surrogateN)));
else
    surrN = 200;
end
...
% Keep coherence computation at the analysis sampling rate (e.g., 1000 Hz).
% Internal decimation here would break the expected fs consistency.
downStep = 1;
fsC = fs / downStep;
...
function writeSourceFinalTable(pathCsv, summaryRows, summaryHeader)
header = {'Run','Phase','Trials','Mean samples','Mean sec','Synergies','VAF','FDR q min', ...
    'Beta mean','Beta peak','Beta Peak Hz','zCoh mean','zCoh peak'};
...
needed = {'pair','phase','status','n_trials','n_synergies','vaf_global','eeg_mode','out_dir'};
for i = 1:numel(needed)
    if ~isKey(idx, needed{i})
        writeCsv(pathCsv, header, {});
        return;
    end
end
...
for i = 1:size(summaryRows,1)
    s = summaryRows(i,:);
    status = strtrim(lower(char(string(s{idx('status')}))));
    if ~strcmp(status, 'ok')
        continue;
    end
    eegMode = strtrim(lower(char(string(s{idx('eeg_mode')}))));
...
rowsTmp = {};
sortKey = zeros(0,3); % [subjectNumber, phaseOrder, sideOrder]
for i = 1:size(summaryRows,1)
    s = summaryRows(i,:);
    status = strtrim(lower(char(string(s{idx('status')}))));
    if ~strcmp(status, 'ok')
    ...
    eegMode = strtrim(lower(char(string(s{idx('eeg_mode')}))));
    if hasSourceRows && ~strcmp(eegMode, 'source_net')
        continue;
    end

    pairName = char(string(s{idx('pair')}));
    phase = strtrim(lower(char(string(s{idx('phase')}))));
    outDir = char(string(s{idx('out_dir')}));
    if isempty(outDir)
        continue;
    end

    metricCsv = fullfile(outDir, 'metriche_sinergie.csv');
    trialCsv = fullfile(outDir, 'trial_info.csv');
    [betaMeanMin, betaMeanMax] = readMetricMinMax(metricCsv, 'beta_mean_13_30_hz');
    [betaPeakMin, betaPeakMax] = readMetricMinMax(metricCsv, 'beta_peak_coherence');
    [betaHzMin, betaHzMax] = readMetricMinMax(metricCsv, 'beta_peak_hz');
    [zcohMeanMin, zcohMeanMax] = readMetricMinMax(metricCsv, 'zcoh_beta_mean');
    [zcohPeakMin, zcohPeakMax] = readMetricMinMax(metricCsv, 'zcoh_beta_peak');
    [qMin, ~] = readMetricMinMax(metricCsv, 'surrogate_q');
    meanSamples = meanSamplesFromTrialInfo(trialCsv);
    meanDurSec = meanDurationSecFromTrialInfo(trialCsv);

    [runLabel, subjOrder, sideOrder] = shortRunLabel(pairName, phase);
    phaseOrder = phaseSortOrder(phase);

    rowsTmp(end+1,:) = {runLabel, phase, ...
        fmtIntCell(s{idx('n_trials')}), fmtDec(meanSamples, 1), fmtDec(meanDurSec, 3), ...
        fmtIntCell(s{idx('n_synergies')}), fmtDec(toNum(s{idx('vaf_global')}), 3), fmtDec(qMin, 4), ...
        fmtRange(betaMeanMin, betaMeanMax, 3), ...
        fmtRange(betaPeakMin, betaPeakMax, 3), ...
        fmtRange(betaHzMin, betaHzMax, 2), ...
        fmtRange(zcohMeanMin, zcohMeanMax, 3), ...
        fmtRange(zcohPeakMin, zcohPeakMax, 3)}; %#ok<AGROW>
    sortKey(end+1,:) = [subjOrder, phaseOrder, sideOrder]; %#ok<AGROW>
end

if isempty(rowsTmp)
    writeSourceFinalTableOutputs(pathCsv, header, {});
    return;
end
[~, ord] = sortrows(sortKey, [1 2 3]);
writeSourceFinalTableOutputs(pathCsv, header, rowsTmp(ord,:));
end

function writeSourceFinalTableOutputs(pathCsv, header, rows)
% 1) CSV standard (virgola)
writeCsv(pathCsv, header, rows);

% 2) CSV locale IT (punto e virgola), utile per Excel in italiano
itCsv = replaceExt(pathCsv, '_it.csv');
writeCsv(itCsv, header, rows, ';');

% 3) XLSX (piu' robusto per apertura tabellare)
xlsxPath = replaceExt(pathCsv, '.xlsx');
allRows = [header; rows];
try
    writecell(allRows, xlsxPath, 'FileType', 'spreadsheet');
catch
    % Fallback silenzioso: manteniamo almeno i CSV.
end
end

function p2 = replaceExt(p, newSuffix)
[folder, name, ~] = fileparts(p);
p2 = fullfile(folder, [name newSuffix]);
end

function [mn, mx] = readMetricMinMax(pathCsv, colName)
mn = NaN;
mx = NaN;
if ~isfile(pathCsv)
    return;
...
try
    T = readtable(pathCsv, 'FileType','text', 'Delimiter',',', 'VariableNamingRule','preserve');
catch
    return;
end
...
function v = meanSamplesFromTrialInfo(pathCsv)
v = NaN;
if ~isfile(pathCsv)
    return;
end
...
if ismember('n_samples', vn)
    x = toNumericVector(T.('n_samples'));
elseif ismember('start_sample', vn) && ismember('end_sample', vn)
    x1 = toNumericVector(T.('start_sample'));
    x2 = toNumericVector(T.('end_sample'));
    x = x2 - x1 + 1;
else
    x = [];
end
...
function v = meanDurationSecFromTrialInfo(pathCsv)
v = NaN;
if ~isfile(pathCsv)
    return;
end
...
vn = T.Properties.VariableNames;
if ismember('duration_sec', vn)
    x = toNumericVector(T.('duration_sec'));
elseif ismember('run_median_duration_sec', vn)
    x = toNumericVector(T.('run_median_duration_sec'));
else
    x = [];
end
x = x(isfinite(x) & x > 0);
...
function [label, subjOrder, sideOrder] = shortRunLabel(pairName, phase)
s = lower(char(string(pairName)));
tok = regexp(s, 'sub-(a\d+)', 'tokens', 'once');
if isempty(tok)
    subj = 'RUN';
    subjOrder = inf;
else
    subj = upper(tok{1});
    subjOrder = str2double(regexprep(subj, '^A', ''));
    if ~isfinite(subjOrder)
        subjOrder = inf;
    end
end

side = inferSideLabel(s);
switch side
    case 'dx'
        sideOrder = 0;
    case 'sx'
...
ph = lower(char(string(phase)));
if isempty(side)
    label = sprintf('%s %s', subj, ph);
else
    label = sprintf('%s %s %s', subj, side, ph);
end
end

function side = inferSideLabel(pairLower)
side = '';
if ~isempty(regexp(pairLower, '(^|[_-])dx([_-]|$)', 'once')) || contains(pairLower, 'reachdx')
    side = 'dx';
    return;
end
if ~isempty(regexp(pairLower, '(^|[_-])sx([_-]|$)', 'once')) || contains(pairLower, 'reachsx')
    side = 'sx';
    return;
end
if contains(pairLower, 'destra') || contains(pairLower, 'right')
    side = 'dx';
elseif contains(pairLower, 'sin') || contains(pairLower, 'left')
    side = 'sx';
end
end

function p = phaseSortOrder(phase)
ph = lower(char(string(phase)));
if strcmp(ph, 'pre')
    p = 0;
elseif strcmp(ph, 'post')
    p = 1;
else
    p = 2;
end
...
function s = fmtRange(v1, v2, nDec)
if ~isfinite(v1) || ~isfinite(v2)
    s = '';
    return;
end
fmt = ['%0.' num2str(nDec) 'f-%0.' num2str(nDec) 'f'];
s = sprintf(fmt, v1, v2);
end

function s = fmtDec(v, nDec)
if ~isfinite(v)
    s = '';
    return;
end
...
function writeCsv(pathCsv, header, rows, delim)
